...
User* select_Client(User *liste, int socket_Client){
	User *parcour = liste;
	if(parcour->socketClient == socket_Client){
		parcour->suivant = NULL;
		return parcour;
	}
	else{
		while(parcour->suivant != NULL){
			if(parcour->suivant->socketClient == socket_Client){
				parcour->suivant->suivant = NULL;
				return parcour->suivant;		
			}
			else parcour = parcour->suivant;
		}
	}
	exit(0);
}
			...
			switch(lus)
			{
				...
				case 0 : // la socket est fermée 
					...
					*nbClient -= 1;
					break;
				default: 
					client = select_Client(*liste, poll_message[i].fd);
					ecrits = traite_message(messageRecu, poll_message[i], parametres, matrice, client, ecrits);
					break;
			}
			if(lus >0){
				switch(ecrits)
				{
					...
					default: // envoi de n octets 
						printf("Message bien envoyé au client (%d octets)\n\n", ecrits);
				}			
			}
		}
	}
