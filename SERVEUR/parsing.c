...
#include "header.h"
int traite_message(char *message, struct pollfd poll, Parametre parametres, char ***matrice, User *user){
	int ecrit;
	...
	if( strcmp(split_string, "/setPixel" ) == 0){
		...
		if(position == NULL || color == NULL){
			...
			if( (atoi(parametre[0]) >= parametres.dimensions[0]) || (atoi(parametre[0]) < 0) ){
				...
				//traiter la commande	
				ecrit = check_limite(user, parametres, matrice, parametre, color);
	...
	//utilisation de strncmp et non strcmp pour eviter les probleme avec les acsii allant de 0 à 32
		...
		if(user->chrono != 0){
			...
			sprintf(messageEnvoi, "%d",temp_restant);		
			ecrit = write(poll.fd, messageEnvoi, strlen(messageEnvoi));
		} 
		else{
...
int check_limite(User *user, Parametre parametres, char ***matrice, char **position, char *color){
	int ecrit = 0;
